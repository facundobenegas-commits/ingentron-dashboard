...
// Opciones de conexión a Firebird locales (corriendo dentro del propio servidor de La Serenísima)
const dbOptions = {
    host: '127.0.0.1', // localhost en el servidor Calvo
    port: 3050,
    database: 'C:/ERPCalvoyAsociados/Database/ERPDATABASE.FDB', // Ruta local de Calvo en disco C
    user: 'SYSDBA',
    password: 'masterkey',
    lowercase_keys: true,
    connectTimeout: 5000
...
// Formateo de fecha de cortes históricos
...
// Ejecutar la sincronización de La Serenísima
async function runSynchronization() {
    const timestamp = new Date().toLocaleString();
    console.log(`\n[${timestamp}] 🔄 Iniciando ciclo de sincronización de La Serenísima...`);
    
    let globalData = [];
    
    try {
        const serenisimaQuery = `
        SELECT 
        ...
        console.log("[Firebird] Consultando base de datos local de La Serenísima...");
        const rows = await queryFirebird(serenisimaQuery);
        ...
        console.log(`[Firebird] Éxito: Cargados ${globalData.length} saldos en tiempo real de La Serenísima.`);

        // Extraer cortes históricos semanales de La Serenísima (los miércoles) de forma automática y dinámica
        ...
        console.error("[Firebird] ERROR al extraer saldos de La Serenísima:", dbErr.message);
        return;
    }
    ...
    try {
        ...
        const response = await fetch(RENDER_SYNC_URL, {
            ...
            headers: {
                ...
                'x-sync-origin': 'Serenisima', // Header identificador de origen
        ...
        console.log(`[Render] ¡Sincronización de La Serenísima EXITOSA!`, result);
    } catch (syncErr) {
        console.error("[Render] ERROR al subir datos de La Serenísima:", syncErr.message);
    }
}
...
console.log(`=============================================================`);
console.log(`🔄 INICIANDO CLIENTE SINCRONIZADOR DE LA SERENÍSIMA (CALVO PC)`);
