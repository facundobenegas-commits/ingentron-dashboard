...
// Opciones de conexión a Firebird locales (corriendo dentro del propio servidor Calvo)
const dbOptions = {
...
async function runSynchronization() {
    ...
    console.log(`\n[${timestamp}] 🔄 Iniciando ciclo de sincronización de Aguas...`);
    ...
    try {
        ...
        console.log("[Firebird] Consultando base de datos local...");
        const rows = await queryFirebird(aguasQuery);
        ...
        console.log(`[Firebird] Éxito: Cargados ${globalData.length} saldos de Aguas.`);
    } catch (dbErr) {
        console.error("[Firebird] ERROR al extraer saldos de Aguas:", dbErr.message);
        return;
    }
    ...
    try {
        ...
        const response = await fetch(RENDER_SYNC_URL, {
            ...
            headers: {
                ...
                'x-sync-origin': 'Aguas', // Header identificador de origen
        ...
        console.log(`[Render] ¡Sincronización de Aguas EXITOSA!`, result);
    } catch (syncErr) {
        console.error("[Render] ERROR al subir datos de Aguas:", syncErr.message);
    }
}
