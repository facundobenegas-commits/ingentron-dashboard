...
// Opciones de conexión a SQL Server locales (corriendo dentro del propio servidor Gescom de Trenque Lauquen)
const sqlServerOptions = {
    user: 'usuario',
    password: '',
    server: '127.0.0.1', // localhost en el servidor Gescom
    database: 'idea', // Base de datos del sistema en C:\IDEA
    options: {
        encrypt: false, // Deshabilitar cifrado para la red local
        trustServerCertificate: true,
        instanceName: 'SQLSERVER2022' // Instancia local de SQL Server de Gescom
    },
...
async function runSynchronization() {
    ...
    console.log(`\n[${timestamp}] 🔄 Iniciando ciclo de sincronización de Trenque Lauquen...`);
    ...
    try {
        ...
        console.log("[SQL Server] Consultando base de datos Gescom local (C:\\IDEA)...");
        ...
        console.error("[SQL Server] ERROR al extraer saldos de Trenque Lauquen:", err.message);
        return;
    } finally {
        if (pool) {
            await pool.close();
    ...
    try {
        ...
        const response = await fetch(RENDER_SYNC_URL, {
            ...
            headers: {
                ...
                'x-sync-origin': 'Trenque Lauquen', // Header identificador de origen
        ...
        console.log(`[Render] ¡Sincronización de Trenque Lauquen EXITOSA!`, result);
    } catch (syncErr) {
        console.error("[Render] ERROR al subir datos de Trenque Lauquen:", syncErr.message);
    }
}
...
console.log(`=============================================================`);
console.log(` 🔄 INICIANDO CLIENTE SINCRONIZADOR DE TRENQUE LAUQUEN (GESCOM)`);
